...
from gui_2_jun import Ui_MainWindow
import sys
...
def predOCR(img):
    ...
    if len(boxWithCharector) > 5:
        ...
        valCsv = []
        t3 = threading.Thread(target=waitForSlabToPass)
        t3.start()
        t3.join()
...
def ocr(ocrCameraLink):
    ...
    while True:
        ...
        if _ == False:
            print("OCR camera Video could not be fetched --- Please close browser and try again")
            ocr = cv2.VideoCapture(ocrCameraLink)
            ocr.set(cv2.CAP_PROP_BUFFERSIZE, 0)
            continue
...
def mmPerPix(img_dim,thresh,alar,pixelsPerMetric):
    ...
    mmPpix = ((1/pixelsPerMetric)* (rightSide - leftSide)) - 80 #* 0.9864
...
def slabDim(dimCameraLiink):
    ...
    dim.set(cv2.CAP_PROP_BUFFERSIZE, 0)
    ...
    while(True):
        ...
        width = int(img_dim.shape[1] * scale_percent / 100)
        height = int(img_dim.shape[0] * scale_percent / 100)
        dim = (width, height)
        # resize image
